...
def change_data():
    try:
        try:
            conn = psycopg2.connect(host="localhost",database="libin_bi1", user="admin", password="tms@123")
            cur = conn.cursor()
            conn.autocommit = True
            df = pd.read_excel("lib_file/employee1.xlsx",header=0,sheet_name="employee")
        ...
        for ind,row in df.iterrows():
            ...
            str_username = str_first_name +" "+ str_last_name
            str_email = row['EMAIL']
            str_phone = row['PHONE']
            str_designation = ''
            str_branch_code = ''
            ...
            # import pdb; pdb.set_trace30
            if str_designation != '' and str_branch_code != '' and str_department != '' and str_brand != '' :
                rst_userexists = User.objects.filter(username = str_username)
                if rst_userexists:
                    ...
                    rst_data.set_password('password')
                    rst_data.save()
